...
export default function RoleModal({ open, onClose, role, onSave }) {
  ...
  const scopeOptions = useMemo(
    () => ["GLOBAL", "COMPANY", "BRANCH", "PLATFORM"],
    []
  );
  ...
  useEffect(() => {
    ...
    if (role) {
      ...
      setForm({
        ...
        name: role.name ?? toSnakeCaseKey(displayName),
        description: role.description ?? "",
  ...
  // auto-generate key
  useEffect(() => {
    if (!open || isSystemRole) return;
    setForm((p) => ({ ...p, name: toSnakeCaseKey(p.display_name) }));
  }, [form.display_name, open, isSystemRole]);

  const validate = useCallback((p) => {
    const e = {};
    if (!p.display_name) e.display_name = "Required";
    if (!p.name) e.name = "Required";
    if (!/^[a-z0-9_]+$/.test(p.name)) e.name = "snake_case only";
    return e;
  }, []);
  ...
  return (
    <EntityModal
      ...
      footerLeft={
        <>
          <span>
            Active: <b>{form.is_active ? "Yes" : "No"}</b>
          </span>
          {isSystemRole && (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div>
          <label className="text-sm">Role Key *</label>
          <input
            value={form.name}
            disabled
