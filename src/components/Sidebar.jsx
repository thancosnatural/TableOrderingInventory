...
const FALLBACK_MENU = useMemo(
  ...
  return (
    <>
      ...
      <aside
        className={`hidden md:flex flex-col h-screen sticky top-0 z-20 bg-white border-r transition-all ${
          collapsed ? "w-20" : "w-64"
        }`}
      >
          ...
          {menuItems.map((it) => (
            <div key={it.key} className="mb-1">
              {!it.children ? (
                <Link
                  to={it.to}
                  className={`group flex items-center gap-3 px-2 py-2 rounded-md hover:bg-gray-50 transition-colors ${
                    isActive(it.to) ? "bg-indigo-50 text-indigo-600" : "text-slate-700"
                  }`}
                >
                ...
                <div>
                  <div
                    className={`flex items-center gap-3 px-2 py-2 rounded-md text-slate-600 ${
                    ...
                    <div className="ml-4 mt-1 space-y-1">
                      {it.children.map((c) => (
                        <Link
                          key={c.key}
                          to={c.to}
                          className={`block px-3 py-1 rounded-md text-sm hover:bg-gray-50 ${
                            isActive(c.to) ? "bg-indigo-50 text-indigo-600" : "text-slate-600"
                          }`}
                        >
                          {c.label}
                        </Link>
      ...
      <div className={`md:hidden ${mobileOpen ? "" : "pointer-events-none"}`}>
        ...
        <div
          className={`fixed inset-y-0 left-0 z-40 w-72 bg-white shadow-xl transform transition-transform ${
