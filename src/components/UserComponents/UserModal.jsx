...
function EyeIcon({ open }) {
  return open ? (
    ...
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path
        d="M10.73 5.08A10.43 10.43 0 0 1 12 5c6.5 0 10 7 10 7a18.16 18.16 0 0 1-3.22 4.52"
        stroke="currentColor"
        strokeWidth="2"
      />
      <path
        d="M6.61 6.61A18.65 18.65 0 0 0 2 12s3.5 7 10 7a10.64 10.64 0 0 0 3.13-.46"
        stroke="currentColor"
        strokeWidth="2"
      />
      <path
...
export default function UserModal({
  ...
  useEffect(() => {
    ...
    if (user) {
      const roleId =
        user.role_id ?? user?.roles?.[0]?.id ?? user?.role?.id ?? user?.roleId ?? "";
  ...
  const validate = useCallback(
    (payload) => {
      ...
      if (!isEditing) {
        ...
        else if (p.length < 6) e.password = "Min 6 characters.";

        if (!c) e.confirm_password = "Confirm password is required.";
        else if (p && c && p !== c) e.confirm_password = "Passwords do not match.";
      }
        ...
        <div className="bg-white sm:rounded-xl sm:shadow-lg w-full h-[92vh] sm:h-auto max-h-[92vh] flex flex-col overflow-hidden">
          ...
          <div className="overflow-auto px-4 py-4 sm:py-6" style={{ paddingBottom: 112 }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {BASE_FIELDS.map((field) => {
                ...
                // ✅ email editable even in edit mode
                const disabled = false;
              ...
              {!isEditing ? (
                <>
                  <PasswordField
                    ...
                    onToggleShow={() =>
                      setShowPwd((p) => ({ ...p, password: !p.password }))
                    }
                    onChange={(e) =>
              ...
              <div className="md:col-span-2">
                ...
                {errors.role_id && (
                  <div className="text-xs text-red-600 mt-1">{errors.role_id}</div>
                )}
              </div>
              ...
              {requiresCompany ? (
                <div className="md:col-span-2">
                  ...
                  {errors.company_id && (
                    <div className="text-xs text-red-600 mt-1">{errors.company_id}</div>
                  )}
                </div>
              ) : null}
          ...
          >
            <div className="hidden sm:flex items-center gap-3 text-sm text-gray-600">
              <div>
                Active:{" "}
                <span className="font-medium ml-1">{form.is_active ? "Yes" : "No"}</span>
              </div>
              <div>
                Blocked:{" "}
                <span className="font-medium ml-1">{form.is_blocked ? "Yes" : "No"}</span>
              </div>
            </div>
